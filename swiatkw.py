...
class GlOknow(QtGui.QWidget):
    ...
    def __init__(self, s):
        ...
        self.setGeometry(100, 100, s.getB()*self.rozmiar, s.getA()*self.rozmiar)
        ...
        for x in range(self.s.getB()):
            for y in range(self.s.getA()):
                if not self.s.istniejeOrganizm(x, y):
                    qp.drawPoint((x+0.5)*self.rozmiar, (y+0.5)*self.rozmiar)
        qp.end()
